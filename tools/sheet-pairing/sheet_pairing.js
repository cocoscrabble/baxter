// GENERATED by tools/sheet-pairing/build.py — do not edit.
//
...
//
// Dropped from Code.ts (21 declarations, all sheet I/O or UI):
//   _show_high_game, _show_high_spread, _show_loss, _show_spread, _show_win, calculateStandings, collectEntrants, collectFixedPairings, collectResults, collectRoundPairings, collectSettings, collectSheetData, getOrCreateSheet, lastResultTimestamp, onOpen, outputDiags, outputPairingLog, outputPairings, outputPlayerStandings, outputStatistics, processSheet
//
// Kept: 62 declarations, byte-for-byte apart from 3
...
//   - pairSwissHelper: mergeBottom returns early once a single group is left, so the loop spins forever on a field small enough to collapse to one group
//   - pairSwissHelper: the retry path only ever raises the repeat limit, which never unlocks a distance-filtered edge, so a distance too narrow to admit a perfect matching loops forever
//   - promote: promote2 can walk past the end of the group list; the original tests for undefined, logs, and then dereferences it anyway
