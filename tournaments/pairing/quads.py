...
def _quad_standings(pd: PairingData, rp: RoundPairing) -> list:
...
def quad_position(rp, round_pairings) -> int:
    """Return the 0-based position of rp within its run of same-strategy, same-start_round entries."""
    pos = 0
    for r in round_pairings:
        if r.pairing == rp.pairing and r.start_round == rp.start_round:
            if r.round == rp.round:
                return pos
            pos += 1
    return 0  # fallback when round_pairings context is unavailable
...
Pairings4 = [[[0, 3], [1, 2]], [[0, 2], [1, 3]], [[0, 1], [2, 3]]]
...
def group_position_pairs(group, pos) -> list[list[int]]:
    if len(group) == 4:
        return Pairings4[pos]
    else:
        return Pairings6[pos]
...
def pair_clustered_quads(pd: PairingData, rp: RoundPairing) -> Pairings:
    ...
    standings = _quad_standings(pd, rp)
